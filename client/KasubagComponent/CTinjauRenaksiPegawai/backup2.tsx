...
function Row(props) {
  ...
  const { row, setPegawai, nip } = props;
  ...
  const btnTolak = () => {
    console.log(arrTolak);
    // let moTrima = [];
    // moTrima = arrTolak.filter((elA) => {
    //   return setPegawai.some((elB) => elA["value"] == elB["id_renaksi"]);
    // });
    // moTrima.map((item) => {
      ...
      <>
        ...
        {row.ditolak == "Kasubid" && row.kirim_ke == "Kasubid" ? (
          <>
            ...
            <TableCell style={{ padding: 0, width: 2000 }} colSpan={8}>
              <Collapse sx={styleCollapse} in={open} timeout="auto">
                <div className={styles.wrapperContentModal}>
                  <div className={styles.contentFeedback}>
                    ...
                    <div>
                      {/* <p>"{row.ket_admin}"</p> */}
                      <p className={styles.feedback}>{row.ket_pegawai}</p>
                    </div>
                  </div>
                  <button className={styles.btnFeedback} onClick={openModal}>
                    <p>Tanggapan</p>
                  </button>
                  <Modal
                    isOpen={modalIsOpen}
                    ...
                    contentLabel="Example Modal"
                  >
                    <h2 className={styles.headerTxtModal}>Feedback</h2>
                    <Gap height={20} width={0} />
                    <input
                      className={styles.inputBuktiLap}
                      placeholder="Tambah keterangan"
                    ...
                    <div style={{ display: "flex", marginLeft: 383 }}>
                      ...
                      <button
                        onClick={() =>
                          Axios.post("http://localhost:3001/tanggapan", {
