...
app.get("/cakin", (req, res) => {
  const sqlSelect =
    "SELECT pegawai.nama, pegawai.jabatan, pegawai.bidang, pegawai.sub_bidang, cakin.bulan, cakin.jumlah_kegiatan, cakin.lampiran_disubmit, cakin.lampiran_bsubmit, cakin.hasil_kinerja, cakin.id_cakin, cakin.nip, pegawai.nip FROM cakin INNER JOIN pegawai ON cakin.nip=pegawai.nip ORDER BY cakin.bulan ASC";
...
app.get("/kabidAmbilRenaksiMJD", (req, res) => {
  const sqlSelect =
    "SELECT data_renaksi.id_renaksi, data_renaksi.kegiatan, data_renaksi.sub_kegiatan, data_renaksi.kirim_ke, data_renaksi.tupoksi_tambahan, data_renaksi.tupoksi_inti, data_renaksi.status, data_renaksi.files, data_renaksi.program, data_renaksi.end_date, data_renaksi.start_date, data_renaksi.nip, data_renaksi.ket_pegawai, pegawai.nama, pegawai.sub_bidang, pegawai.jabatan, pegawai.foto, thl.nama_thl, thl.thl FROM data_renaksi INNER JOIN pegawai ON data_renaksi.nip=pegawai.nip LEFT OUTER JOIN thl ON data_renaksi.thl=thl.thl WHERE data_renaksi.status = 'Menunggu Jadwal Diubah' AND data_renaksi.kirim_ke = 'Kabid' ";
  db.query(sqlSelect, (err, result) => {
    res.send(result);
  });
});
...
app.post("/adminMenolakRenaksiMJD", (req, res) => {
  ...
  const sqlUpdate =
    'UPDATE data_renaksi SET status = "Sementara", kirim_ke = "", req_start_date = "", req_end_date = "", ket_admin = ?  WHERE id_renaksi = ?';
...
app.post("/adminMenolakRenaksiDihapus", (req, res) => {
  ...
  const sqlUpdate =
    'UPDATE data_renaksi SET status = "Sementara", kirim_ke = "", ket_admin = ?  WHERE id_renaksi = ?';
  let data = [ketAdmin, idRenaksi];
  db.query(sqlUpdate, data, (err, result) => {
    console.log(result);
  });
